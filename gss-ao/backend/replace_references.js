...
async function run() {
  try {
    ...
    const insertIndex = 5; 
    console.log(`Removing ${copiedPages.length} existing reference pages starting at index ${insertIndex}...`);
    
    // Remove the previously inserted pages
    for (let i = 0; i < copiedPages.length; i++) {
      livePdfDoc.removePage(insertIndex); // We remove at insertIndex repeatedly because the array shifts
    }
